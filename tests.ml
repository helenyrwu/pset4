 ...
 *
 * Information about testing the modules
 *)
 ...
 * You as the programmer may know that Leaf and Branch are constructors of
 * the IntTree.tree type, but in terms of OCaml's type abstraction, the
 * /client/ of the IntTree module cannot know this, because the module signature
 * doesn't reveal it. The client of the IntTree module can only know what
 * is provided by the signature: "BINTREE with type elt = C.t" found in the
 ...
 *
 * There is no one ideal solution to this dilemma, but we're providing you
...
let test () =
  ...
  assert((heapsort []) = []);;
  assert((heapsort [1]) = [1]);;
  assert((heapsort [0;-3;1;2;5;1;9;-4]) = [-4; -3; 0; 1; 1; 2; 5; 9]);;
  ...
  assert((treesort []) = []);;
  assert((treesort [1]) = [1]);;
  assert((treesort [0;-3;1;2;5;1;9;-4]) = [-4; -3; 0; 1; 1; 2; 5; 9]);;
  assert((treesort [1;2;3;4]) = [1;2;3;4]);;
  assert((treesort [4;3;2;1]) = [1;2;3;4]);;

  (* selectionsort tests *)
  assert((selectionsort []) = []);;
  assert((selectionsort [1]) = [1]);;
  assert((selectionsort [0;-3;1;2;5;1;9;-4]) = [-4; -3; 0; 1; 1; 2; 5; 9]);;
  assert((selectionsort [1;2;3;4]) = [1;2;3;4]);;
  assert((selectionsort [4;3;2;1]) = [1;2;3;4]);;
 ...
 *   Foo.run_tests ()
 * by placing in this file an appropriate invocation (as the IntTree.run_tests
 * above).
 *
